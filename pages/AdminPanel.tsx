...
import { ArrowLeft, Check, LayoutGrid, Bell, PhoneCall, ShoppingBag, Droplet, Headphones, Star, RotateCcw, Sprout, GraduationCap, Award } from 'lucide-react';
...
import AdminTalentHub from '../components/admin/AdminTalentHub';

type AdminView = 'home' | 'notice' | 'cards' | 'emergency' | 'market' | 'blood' | 'helpdesk' | 'popular' | 'agriculture' | 'education' | 'talentHub';
...
const AdminPanel: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const labels = useMemo(() => {
    return {
      ...
      educationTitle: language === 'bn' ? 'শিক্ষা ও তরুণ' : 'Education & Youth',
      talentHubTitle: language === 'bn' ? 'ট্যালেন্ট হাব' : 'Talent Hub',
  ...
  const adminMenuIcons = [
    ...
    { id: 'education', label: labels.educationTitle, icon: GraduationCap, color: 'bg-indigo-500', light: 'bg-indigo-50' },
    { id: 'talentHub', label: labels.talentHubTitle, icon: Award, color: 'bg-purple-500', light: 'bg-purple-50' },
  ] as const;
  ...
  return (
    <div className="pb-32 animate-in fade-in duration-500">
      ...
      <div className="max-w-4xl mx-auto px-4 -mt-6 relative z-20">
        ...
        { activeView === 'education' && <AdminEducation {...sharedProps} /> }
        { activeView === 'talentHub' && <AdminTalentHub {...sharedProps} /> }
